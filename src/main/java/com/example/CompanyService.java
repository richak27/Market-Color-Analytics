...
public class CompanyService {
	...
	public List<Float>calAverageVol(Company company)
	{
		float sum_vol_pre = 0;
		float sum_vol_post = 0;
		int sizeofpre = 0;
		List<Stock> stocks = company.getStocks();
		for (Stock stock: stocks) {
			if(stock.getPeriod()== true) {
				sizeofpre++;
				sum_vol_pre += stock.getVolume();	
			}
			else {
				sum_vol_post += stock.getVolume();
			}
		}
		
		return Arrays.asList( (sum_vol_pre/sizeofpre),(sum_vol_post/(stocks.size()-sizeofpre)));
		
		
	}
	
	public List<Float> calAverageStock(Company company)
	{
		float sum_close_pre = 0;
		float sum_close_post = 0;
		int sizeofpre = 0;
		List<Stock> stocks = company.getStocks();
		for (Stock stock: stocks) {
		...
		return Arrays.asList( (sum_close_pre/sizeofpre),(sum_close_post/(stocks.size()-sizeofpre)));
